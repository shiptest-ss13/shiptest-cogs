...
def tgs_encode(obj):
    ...
    if isinstance(obj, timedelta):
        return tgs_timedelta_enc(obj)
    if isinstance(obj, TgsModelBase):
        return obj.to_dict()
    return obj
...
class TgsModelBase:
    ...
    def from_dict(self, dict):
        ...
        if not self._base_json:
            self._base_json = JSONEncoder().encode(dict)
        return self
...
class TgsModel_UserApiBase(TgsModel_UserModelBase):
    ...
    def sanitize(self):
        super().sanitize()
        _l = list()
        for oauth in self.OAuthConnections:
            _l.append(TgsModel_OAuthConnection().from_dict(oauth))
...
class TgsModel_Job(TgsModel_EntityId):
    ...
    def sanitize(self):
        ...
        if self.StoppedAt:
            self.StoppedAt = tgs_datetime(self.StoppedAt)
...
class TgsModel_JobResponse(TgsModel_Job):
    ...
    def sanitize(self):
        ...
        if self.CancelledBy:
            self.CancelledBy = TgsModel_UserName().from_dict(self.CancelledBy)

    def state(self) -> JobState:
        if not self.StoppedAt:
            return JobState.Running
        if self.ErrorCode:
            return JobState.Errored
        if self.Cancelled:
...
class TgsModel_ByondInstallResponse(TgsModel_FileTicketResponse):
    ...
    def sanitize(self):
        super().sanitize()
        if self.InstallJob:
            self.InstallJob = TgsModel_JobResponse().from_dict(self.InstallJob)
...
class TgsModel_ChatBotApiBase(TgsModel_ChatBotSettings):
    ...
    def sanitize(self):
        ...
        for entry in self.Channels:
            _l.append(TgsModel_ChatChannel().from_dict(entry))
        self.Channels = _l
...
class TgsModel_RevisionInformation(TgsModel_RevisionInformation_Internal):
    ...
    def sanitize(self):
        super().sanitize()
        if self.PrimaryTestMerge:
            self.PrimaryTestMerge = TgsModel_TestMerge().from_dict(self.PrimaryTestMerge)
        _l = list()
...
class TgsModel_CompileJobResponse(TgsModel_CompileJob):
    ...
    def sanitize(self):
        super().sanitize()
        if self.Job:
            self.Job = TgsModel_JobResponse().from_dict(self.Job)
        if self.RevisionInformation:
            self.RevisionInformation = TgsModel_RevisionInformation().from_dict(self.RevisionInformation)
...
class TgsModel_DreamDaemonResponse(TgsModel_DreamDaemonApiBase):
    ...
    def sanitize(self):
        super().sanitize()
        if self.ActiveCompileJob:
            self.ActiveCompileJob = TgsModel_CompileJobResponse().from_dict(self.ActiveCompileJob)
        if self.StagedCompileJob:
            self.StagedCompileJob = TgsModel_CompileJobResponse().from_dict(self.StagedCompileJob)
...
class TgsModel_DreamMakerSettings(TgsModelBase):
    ...
    def sanitize(self):
        super().sanitize()
        if self.Timeout:
            self.Timeout = tgs_timedelta(self.Timeout)
...
class TgsModel_InstanceResponse(TgsModel_Instance):
    ...
    def sanitize(self):
        super().sanitize()
        if self.MoveJob:
            self.MoveJob = TgsModel_JobResponse().from_dict(self.MoveJob)
...
class TgsModel_RepositoryResponse(TgsModel_RepositoryApiBase, TgsModel_IGitRemoteInformation):
    ...
    def sanitize(self):
        ...
        if self.RevisionInformation:
            self.RevisionInformation = TgsModel_RevisionInformation().from_dict(self.RevisionInformation)
        if self.ActiveJob:
            self.ActiveJob = TgsModel_JobResponse().from_dict(self.ActiveJob)
